...
router.post('/session/start', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  // Immediately fetch first question (avoids a second round trip)
  const [seenItems, learnedChapters, conceptStats, user] = await Promise.all([
    prisma.adaptiveItem.findMany({ where: { sessionId: session.id }, select: { questionId: true, question: { select: { text: true } } }, take: 200 }),
    prisma.userChapter.findMany({ where: { userId, isLearned: true }, select: { chapterId: true } }),
    prisma.userConceptStat.findMany({ where: { userId }, select: { conceptId: true, chapterId: true, total: true, correct: true }, take: 500 }),
  ...
  const isFocused = focusConceptId !== null && focusConceptId !== undefined;
  ...
  const [rawCandidates, rawAdjacent] = await Promise.all([
    isFocused
      ? fetchCandidates({ conceptId: focusConceptId, isActive: true, ...penPaperFilter })
      : fetchCandidates({ chapterId: { in: chapterIds }, isActive: true, ...penPaperFilter }),
    isFocused ? findAdjacentQuestions([focusConceptId!], seenIds, penPaperFilter) : Promise.resolve([]),
  ]);
  ...
  if (candidates.length === 0) {
    return res.json({ session, done: true, message: 'All questions in learned chapters completed!' });
  }
  ...
  await prisma.adaptiveItem.create({ data: { sessionId: session.id, questionId: chosen.id, order: 1 } });
